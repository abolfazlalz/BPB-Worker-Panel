            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                switch (url.pathname) {
                    ...
                    case '/panel':
                        ...
                        const proxySettings = await env.asali.get('proxySettings', { type: 'json' });
...
const getWarpConfigs = async (env, client) => {
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = `🍯 Asali - WoW ${index / 2 + 1} 🌍`;
...
const updateDataset = async (env, Settings) => {
    ...
    try {
        currentProxySettings = await env.asali.get("proxySettings", { type: 'json' });
    } catch (error) {
        console.log(error);
...
const renderHomePage = async (env, hostName, fragConfigs) => {
    ...
    try {
        proxySettings = await env.asali.get('proxySettings', { type: 'json' });
        warpConfigs = await env.asali.get('warpConfigs', { type: 'json' });
        password = await env.asali.get('pwd');
    } catch (error) {
        console.log(error);
